...
export default function DashboardPage() {
  ...
  return (
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px" }}>
      ...
      {locked && (
        <div style={upsellStyle}>
          <h2 style={{ fontSize: 20, marginBottom: 8 }}>🔒 {checkpoint} is a premium feature</h2>
          <p style={{ color: "#8b949e", marginBottom: 16 }}>
            Pre-Tournament predictions are free. Round-by-round analysis (updated live after
            each round) is available with a subscription.
          </p>
          <button onClick={handleSubscribe} disabled={checkoutLoading} style={subscribeButtonStyle}>
            {checkoutLoading ? "Redirecting to checkout..." : "Subscribe — $9.99/month"}
          </button>
        </div>
      )}
      ...
      {!locked && !loading && !error && preds === null && (
        <div style={{ color: "#f85149" }}>No &lsquo;{checkpoint}&rsquo; predictions found in S3.</div>
      )}
